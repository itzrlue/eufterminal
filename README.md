...
| `write <n>` | Write or append to file |
...
| `open <n>` | Open with default app |
| `count [dir]` | Count files and folders |
| `large [dir] [MB]` | Find files larger than N MB |
| `recent` | Show 20 most recently modified files |
...
| `html [file.html]` | Open HTML in browser |
| `js [file.js]` | Run JavaScript file |
| `calc <expr>` | Math expression evaluator |
| `encode <method> <txt>` | Base64/hex/URL encode/decode |
| `clipboard <act>` | Get/set/clear clipboard |
| `note <act>` | Add/list/clear notes |
| `alias <n> <cmd>` | Create session shortcut |
| `run [file]` | Auto-run any file by type |
| `history [n]` | Show command history |
| `clear-history` | Clear session history |
| `diskusage` | Show disk usage for all drives |
...
| `wifi-list` | Show saved Wi-Fi passwords |
...
| `startup-check` | Show startup programs |
| `activeapps` | Show running applications |
...
**EUF's Terminal - Your power in simple commands.** ⚡
